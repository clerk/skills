...
4. **Current custom-flows API only** — `useSignIn()` / `useSignUp()` from `@clerk/expo` (v3.4+) return `{ signIn, errors, fetchStatus }` and use method-based flows: `signIn.password()`, `signIn.phoneCode.sendCode()`, `signIn.finalize()`. Never generate the legacy API (`isLoaded`, `signIn.create()` + `prepareFirstFactor`/`attemptFirstFactor`, `setActive({ session })`) — it lives at `@clerk/expo/legacy` and is only for maintaining existing legacy code, never for new work.
