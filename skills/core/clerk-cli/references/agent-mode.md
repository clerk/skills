...
| Other listed mutation confirmations (`config patch` / `put`, `api -X POST/PATCH/DELETE`, `enable` / `disable`) | Prompt y/n | **Silently skipped - the mutation executes.** Their gates are `isHuman() && !options.yes`, so agent mode bypasses them: `--yes` is not their safety gate. Preview and target them carefully |
| Native-aware `clerk init` for an existing iOS/macOS Xcode app | Previews and asks before local/remote writes | Requires explicit `--yes` for native mutation. `--prebuilt-auth-ui` and `--sign-in-with-apple` remain separate opt-ins; `--allow-dirty` is separate consent for planned dirty files. The feature is not yet in the published CLI; check `clerk init --help` first |
...
| `clerk init`                                                     | Full interactive scaffold flow                                   | Runs non-interactively. Explicit `--app` or a linked profile uses the real-app auth/link/env flow; an authenticated agent on an accountless-capable framework can create and link a real app. An unauthenticated agent on such a framework can mint an unclaimed accountless app with temporary dev keys. `--accountless` forces that path; `--login` forces the authenticated flow (usage error while signed out in agent mode). Native iOS/macOS does not support accountless mode and needs a real app plus validated account credentials |
...
**Rule of thumb:** pass `--yes` for native-aware Xcode `init` and other mutations that require it, and `--json` for structured output where available. Pass `--app` / `--instance` when you intentionally target a real app. Accountless-capable frameworks can bootstrap without authentication; native iOS/macOS cannot. `--keyless` is a deprecated compatibility alias and should not be used in new instructions.

## Native Apple init and Doctor

This applies only if the installed `clerk init --help` lists native `--dry-run` and `--target`. Check `--prebuilt-auth-ui` and `--sign-in-with-apple` support before using either option. Older installs may lack native support; use the source-guided `clerk-swift` workflow instead of guessing a native CLI command.

For an existing iOS/macOS Xcode application, start with `clerk init --dry-run`. In agent mode its JSON plan is local and read-only; inspect `status` and blockers rather than trusting exit code alone (`blocked` can exit `0`). Mirror an explicitly requested `--prebuilt-auth-ui` or `--sign-in-with-apple` flag in both dry-run and apply so the reviewed plan includes it. Do not pass `--app` or `--app-id-prefix` to dry-run. If the user authorized the resulting setup, run `clerk init --yes`, adding `--target` only for ambiguous targets. The CLI can obtain the development publishable key for a proven fresh SwiftUI app; do not demand the key or run `clerk env pull` for native source. If an existing `Clerk.configure` uses a custom key source, the CLI leaves it untouched and agent mode requires `--app <id>` to select the application. Add `--app-id-prefix` only if a new registration lacks reliable literal or existing-registration evidence. Never infer it from `DEVELOPMENT_TEAM`.

Prebuilt AuthView scaffolding and native Sign in with Apple are independent, explicit choices: use `--prebuilt-auth-ui` or `--sign-in-with-apple` only when requested. `--yes` alone does not select either. The prebuilt option preserves established UI and edits only a pristine starter. If the plan blocks on a visionOS-containing or Catalyst target, custom project generator, uncertain ownership, or another unsupported shape, stop automatic mutation and give manual setup guidance. Re-running a completed target is intended to be a no-op.

Dry-run does not contact the linked Clerk instance. A later authenticated preview may include a local Apple entitlement if an already enabled method would appear in `AuthView`; inspect that preview too. Do not confuse this with opting into a new Apple provider connection.

Use `clerk doctor --json` afterward for read-only native diagnostics, passing the same `--target` when selection is ambiguous. It checks local Clerk wiring and linked Native Application state but does not run Xcode package resolution, builds, or Simulator execution. `doctor --fix` is ignored in agent mode; build/run validation belongs to the project's Xcode workflow.
...
| --------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `clerk doctor --json`                         | `[{name, status, message, detail?, remedy?, fix?}]`; native-aware builds include iOS/macOS checks |
| `clerk init --dry-run` (native-aware agent mode) | Read-only native Apple inspection and setup plan; branch on `status`, not exit code alone |
...
- **Don't run `clerk auth login` just to bootstrap an accountless-capable project** - it is not required. Native iOS/macOS setup needs a real app and account credentials; see [auth.md](auth.md#accountless-operating-without-an-account) for which other commands remain account-only. Log in when needed to claim an app, link a real app, or use account-only commands.
