...
| `clerk init`                                                     | Full interactive scaffold flow                                   | Runs non-interactively. Explicit `--app` or a linked profile uses the real-app auth/link/env flow; an authenticated agent on a keyless-capable framework creates a real app and links it. **An unauthenticated agent defaults to keyless**: it mints an unclaimed keyless app with temporary dev keys — no login, no flag, no browser. `--keyless` forces keyless over both things that would otherwise win — being signed in, and an existing linked profile — which makes it the escape hatch when a stale link would otherwise send you into a browser login; `--login` forces the authenticated flow (usage error while signed out in agent mode). Only a non-keyless framework with no app target prints manual setup guidance. |
...
- **Don't assume the keyless branch means "a safe throwaway app."** It follows the local `sk_` key, not a verified unclaimed application: it fires whenever there is no `--app` and no linked profile, and accepts `sk_live_` the same as `sk_test_`. In an unlinked project holding a production key, an unguarded `enable orgs` / `config patch` mutates production with no confirmation in agent mode. Pass `--app <id>` when you mean a specific real app.
