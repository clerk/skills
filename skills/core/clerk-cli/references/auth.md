...
`clerk init` mints a claimable, accountless app (which saves temporary development keys) when unauthenticated — no login, no platform key, no browser. That covers agent runs and human bootstraps on frameworks with accountless support; a signed-out human in an *existing* project gets the login flow unless they pass `--accountless` (`--keyless` remains a deprecated compatibility alias).
