...
For setup or `init` requests, read [references/setup.md](references/setup.md) first and follow it as the source of truth. Its package-runner and init-first flow takes precedence over the general operational prerequisites below.
...
Except when following the setup reference, verify the CLI is authenticated, linked, and healthy before running other Clerk commands in a session:
...
- **Follow the generated setup reference for `init`.** Its accountless behavior, supported-framework conditions, command forms, and claim ordering are synchronized from clerk-docs; do not reconstruct a separate setup flow from this operational reference.
- **`--keyless` is a deprecated compatibility alias.** Use `--accountless` in all new commands and instructions.
- **`--fresh` is destructive.** It replaces the temporary app and overwrites the env keys and legacy `.clerk/keyless.json` breadcrumb with no prompt, orphaning the previous app and its users. Never pass it just to re-run `init`.
...
- [references/setup.md](references/setup.md) - generated setup and initialization flow; read for every setup or `init` request.
