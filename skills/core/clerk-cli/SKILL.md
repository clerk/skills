...
| `clerk migrate …`             | Export users from Auth0, Supabase, Firebase, Auth.js, Better Auth, WorkOS or another Clerk instance (`migrate export`), import them (`migrate import`), undo (`migrate delete`), plus `logs`, `settings`, `transformers list`. **`migrate import` writes immediately in agent mode.** Load the `clerk-migrate` skill.| (see the `clerk-migrate` skill)                                                                                                                                                  |
...
- **Only `unlink` and three `migrate` commands actually require `--yes`.** `migrate delete`, `migrate logs clean` and a bare `migrate settings clear` also exit with a usage error without it. Every other confirmation gate is written as `isHuman() && !options.yes`, so agent mode skips it outright: the mutation executes with no prompt and no error. Passing `--yes` is harmless but changes nothing. Do not treat it as a safety gate - `--dry-run` is the real one.
