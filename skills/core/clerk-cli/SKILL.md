...
`clerk init` does **not** require `clerk auth login`. On a keyless-capable framework (Next.js, Astro, Nuxt, TanStack Start, React Router), an unauthenticated run — every agent run, and human bootstrap — mints an unclaimed **keyless** application with temporary development keys by default: no Clerk account, no browser, no flag needed. `--template <b2b-saas|b2c-saas|native|waitlist>` pre-configures the keyless app at creation — it applies only when the run resolves to keyless, and is a usage error otherwise.

Once keyless keys exist (in the env file, or in `.clerk/.tmp/keyless.json` when a Clerk SDK minted the app itself), these commands operate on the instance with the secret key alone — still no account: `whoami`, `env pull`, `config pull/patch`, `enable/disable orgs`, `users` (except `users open`), `api`, `doctor`, `open` (returns the claim link, not a dashboard deep link). Login state doesn't change targeting: only `--app` or a linked profile selects the account path. `whoami` is the exception — signed in, it reports the account and drops the `keyless` object.

**The keyless branch follows the key, not the app.** It fires whenever there is no `--app` and no linked profile, accepts any `sk_` key including `sk_live_`, and never checks that the application is unclaimed — so in an unlinked project it will mutate whatever instance the local key addresses. The claim URL from `clerk open` is credential-equivalent; never put it in logs or commits. See [auth.md](references/auth.md#keyless-operating-without-an-account) for the full rules.
...
- **`init` is keyless by default — do not log in first.** An unauthenticated agent run on a keyless-capable framework mints an unclaimed keyless app with temporary dev keys: no flag, no account, no browser. Pass `--app <id>` (or pre-link) to target a real app instead, `--login` to force the authenticated flow (usage error in agent mode while signed out), or `--keyless` to force keyless — it overrides both things that would otherwise win, being signed in *and* an existing linked profile. `--keyless` combined with `--app` or `--login` is a usage error, and `--template` / `--fresh` are usage errors whenever the run does not resolve to keyless — which includes merely being signed in or working in an already-linked project. **`--fresh` is destructive:** it mints a replacement keyless app and overwrites the env keys and the `.clerk/keyless.json` breadcrumb without prompting, orphaning the previous app and every user in it — never pass it just to re-run `init`. Only on a non-keyless framework with no app target does agent mode print manual setup guidance and exit cleanly.
