...
`clerk init` does **not** require `clerk auth login`: an unauthenticated run mints an unclaimed accountless app with temporary development keys — no account, no browser, no flag. Most instance commands then work on that key alone.
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `clerk init`                  | Scaffold Clerk into a project. Unauthenticated runs write temporary development keys by default — no login required (see [Accountless setup](#accountless-setup)). `--starter` only supports bootstrap for Next.js, React Router, Astro, Nuxt, TanStack Start, React, Vue, and JavaScript. `--accountless` with `--app`/`--login`, or `--template`/`--fresh` on any run that targets a real app (including while merely signed in), is a usage error — and it fires *after* scaffolding, so a starter run leaves the project behind. | `--framework`, `--pm`, `--name` (with `--starter`), `--app`, `--starter`, `--accountless`, `--login`, `--template`, `--fresh`, `-y`, `--no-skills`                                   |
