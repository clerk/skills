...
description: >-
  ...
  "clerk env pull", "clerk doctor", "clerk deploy", "clerk api", native
  iOS/macOS setup, or any ad-hoc
...
> This skill targets clerk `latest`. If `clerk --version` disagrees with the latest available CLI, refresh it with `clerk update`, or invoke the latest through a package runner such as `bunx clerk@latest`. The binary is always the source of truth, so run `clerk <command> --help` to verify anything this skill claims. Some installed versions lack native iOS/macOS init and Doctor support; check capabilities instead of assuming an upgrade provides them.
...
For an existing linked project, verify the CLI's health before other management commands:
...
On a framework with accountless support, `clerk init` does **not** require `clerk auth login`: an unauthenticated bootstrap with no `--app`, or an unauthenticated agent run with no `--app` or existing project link, mints an unclaimed accountless app with temporary development keys — no account, no browser, no flag. (A signed-out human re-running it in an *existing* project gets the login flow instead — `--accountless` forces the temporary-keys path.) Native iOS/macOS projects do not support this accountless path. Most instance commands then work on an accountless key alone.
...
## Native iOS/macOS setup (native-aware CLI only)

Use this workflow only when the installed `clerk init --help` exposes native `--dry-run` and `--target` support. Check optional flags before using them. Do not assume an update makes native support available. Without it, use the source-guided `clerk-swift` workflow and the project's normal Xcode setup.

For an existing supported Xcode app, a human can start with bare `clerk init`: it detects the target, previews local and remote changes, and prompts before writing. In agent mode, inspect with `clerk init --dry-run` and read the JSON plan's `status` before any authorized apply with `clerk init --yes`; a `blocked` plan can still exit `0`. Prebuilt AuthView and native Sign in with Apple are separate explicit opt-ins; if requested, include their flags in both the dry-run preview and apply. Do not pass `--app` or `--app-id-prefix` to dry-run. Do not demand a publishable key or run `clerk env pull` for native source: the CLI fetches the development public key for a proven fresh SwiftUI target. After setup, `clerk doctor --json` diagnoses local and remote state without building or launching the app. A missing authentication-flow finding means core setup succeeded but sign-in UI remains to be implemented; do not report complete authentication until that is addressed. See [references/agent-mode.md](references/agent-mode.md#native-apple-init-and-doctor) for agent-only flags, blockers, and manual handoffs.
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `clerk init`                  | Scaffold Clerk into a project. Accountless-capable frameworks can use temporary development keys without login (see [Accountless setup](#accountless-setup)). `--starter` only supports bootstrap for Next.js, React Router, Astro, Nuxt, TanStack Start, React, Vue, and JavaScript. `--accountless` with `--app`/`--login`, and `--template`/`--fresh` with `--login`, are usage errors before anything is scaffolded. `--template`/`--fresh` on a run that *resolves* to a real app (including while merely signed in) errors only after strategy resolution, so a starter run leaves the scaffolded project behind. Native-aware CLI builds also support existing iOS/macOS Xcode projects; see [Native iOS/macOS setup](#native-iosmacos-setup-native-aware-cli-only). | Common: `--framework`, `--pm`, `--name` (with `--starter`), `--app`, `--starter`, `--accountless`, `--login`, `--template`, `--fresh`, `-y`, `--no-skills`. Native-aware only: `--dry-run`, `--json`, `--target`, `--allow-dirty`, `--app-id-prefix`, `--sign-in-with-apple`, `--prebuilt-auth-ui`. |
...
| `clerk doctor`                | Health check (CLI version, login, link, env, config, completion; plus host-execution probe in agent mode). Without `--fix` it is read-only. Native-aware builds diagnose selected iOS/macOS Xcode targets and remote Clerk state without building or launching the app. | `--json`, `--spotlight`, `--verbose`, `--fix`; native-aware only: `--target` |
...
- **Accountless-capable `init` needs no login.** An unauthenticated agent run can mint an unclaimed accountless app with temporary dev keys: no flag, no account, no browser. `--app <id>` or a pre-link targets a real app instead; `--accountless` forces the temporary-keys path over both a session and an existing link. `--keyless` remains a deprecated compatibility alias. Native iOS/macOS setup is different: it requires a real application and validated account credentials, or stops with a handoff. Flag exclusivity is in the command table above.
...
- **`unlink` and native-aware Xcode `init` require `--yes` in agent mode.** Native `--yes` authorizes only the planned local/remote changes; it does not opt into prebuilt UI or Sign in with Apple. Other confirmation gates listed in [references/agent-mode.md](references/agent-mode.md) are skipped in agent mode, so preview and target those mutations carefully.
