...
- Unclaimed apps created by `clerk init` are configurable without an account — see the [no-account command table](../clerk-cli/references/auth.md#operating-without-an-account) for which commands work and which need a claimed app.
