...
For fresh setup or `init` requests, read [references/setup.md](references/setup.md) first and follow it as the source of truth for the shared setup flow. Keep the version-specific guidance and Core 2 callouts in this skill when working with an existing Clerk integration, and do not upgrade its SDK generation unless the user asks.
