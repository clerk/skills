...
Claude Code plugin skills are namespaced. After plugin install, start with
`/clerk-skills:clerk`; standalone Agent Skills and Codex installs expose
`clerk`.

## Skills

The bundle contains 21 flat skills. Start with `clerk` when you want the
...
The validator checks JSON manifests, the 21 flat skill directories, the
`skills/clerk` orchestrator, skill frontmatter, plugin `skills/` discovery,
Cursor-compatible `mcp.json`, and Clerk MCP wiring. Evaluation workspaces
generated by skill tooling are ignored via `*-workspace/`.

## Compatibility Strategy

This repository is the single source of truth for the distributed Clerk agent
bundle. Claude, Cursor, and Codex manifests all point at the same flat
`skills/` directory. Root `.mcp.json` and `mcp.json` are both committed so
Claude-style and Cursor-style MCP discovery resolve the same Clerk MCP server.

Agents, commands, hooks, and provider-specific duplicate skill trees should only
be added after there is a validated artifact to ship. Until then, the bundle
stays scoped to skills plus MCP configuration.
