...
Cross-check `skills/clerk-cli/` in this repository against the actual Clerk CLI source and propose precise edits where the skill has drifted. The CLI is the source of truth; the skill is maintainer documentation that must track it.
...
- **Target skill**: `skills/clerk-cli/SKILL.md` and `skills/clerk-cli/references/*.md`.
...
- Path, such as `skills/clerk-cli/SKILL.md` or `skills/clerk-cli/references/agent-mode.md`.
...
- Keep `skills/clerk-cli/SKILL.md` near the 500-line guidance. Move detailed material to `references/` instead of bloating the main skill.
...
- [drift|gap|polish] <one-line description>
  ...
  - target: skills/clerk-cli/SKILL.md:<line>
